...
function loadReady(){
	$(".editorInput").click(function(){
		...
		var name="Menu_"+nameDiv.split("_")[1];
		$(".rtfMenu[name='"+name+"']").fadeIn();
	});

	$(".editorInput").keyup(function(){
		var texto=$(this).html();
		var name=$(this).attr("name");
		$("textarea[name='"+name+"']").val(texto);
	...
	$("li[name='linkLi']").mouseover(function(){
		saveRange($(this).closest(".editorDiv").children(".editorInput"));
	});
	
	$("input[name='inputLink']").focus(function(){
		if(range != null){
			selectText(range.start, range.end, $(this).closest(".editorDiv").children(".editorInput"));
		}
	});
}
	...
	$(document).mouseup(function (e)
		{
			...
			{
				container.children(".editorInput").removeClass("editable").attr("contenteditable", "false");
				container.children(".rtfMenu").fadeOut();
...
function createDiv(x, y, h, w, elem){
	var name=$(elem).attr("name");
	var div="<div class='editorDiv' name='div_"+name+"'><div class='editorInput' name='input_"+name+"' style='height:"+h+"px; width:"+w+"px; top:"+y+"px; left:"+x+"px;'>"+$(elem).text()+"</div></div>";
	$(div).insertBefore(elem);
	$(elem).hide();
	createMenu(x, y, h, w, name)
}
...
function createMenu(x, y, h, w, name){
	if(h<200){
		...
		var div='<div class="rtfMenu " name="Menu_'+name+'" style="height:'+h+'px;">	<ul>';
	}
	...
	div +='<li name="linkLi"><button><span class="glyphicon glyphicon-link"></span></button><ul class="image link"><li><input type="text" name="inputLink"><button format="createlink"><span class="glyphicon glyphicon-link"></span></button></li></ul></li>';
...
function selectText(start, end, elem){
	var mainDiv = elem[0];
	var startNode = mainDiv.firstChild;
	var endNode = mainDiv.lastChild;

	var range = document.createRange();
	range.setStart(startNode, start); // 6 is the offset of "world" within "Hello world"
	range.setEnd(endNode, end); // 7 is the length of "this is"
...
function saveRange(editor){
	var position=getSelectionCharOffsetsWithin($(editor)[0]);
